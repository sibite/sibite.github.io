...
BezierCurve = function(pointsQuantity, degree, ...points) {
  ...
  this.getYofX = function(x)  {
    let leftIndex = this.pointList.findIndex(
      (point, index) => point[0] == x || point[0] < x && (this.pointList[index+1] && this.pointList[index+1][0]) > x
    );
    if (leftIndex == -1) {
      return x < this.pointList[0][0] ? this.pointList[0][1] : this.pointList[this.pointList.length-1][1];
    }
...
BezierCurve.LINEAR = new BezierCurve(2, 1, [0, 0], [1, 1]);
BezierCurve.EASE = new BezierCurve(30, 3, [0, 0], [0.5, 0], [0.5, 1], [1, 1]);
BezierCurve.EASE_IN = new BezierCurve(30, 3, [0, 0], [0.5, 0], [1, 1], [1, 1]);
BezierCurve.EASE_OUT = new BezierCurve(30, 3, [0, 0], [0, 0], [0.5, 1], [1, 1]);
BezierCurve.EASE_OUT_CUBIC = new BezierCurve(30, 3, [0, 0], [0.215, 0.61], [0.355, 1], [1, 1]);
